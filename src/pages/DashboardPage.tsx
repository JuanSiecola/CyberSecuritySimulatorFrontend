...
export default function DashboardPage() {
    const { empresa, actividadPendiente, pendientes, recientes, cargando, error, seleccionarActividad, avanzarTurno, resolver } = useDashboard()
    ...
    return (
        <div className="mx-auto flex w-full max-w-[1500px] flex-col gap-5 px-4 pb-8 pt-5 sm:px-6 lg:px-8">
            <div className="flex items-end justify-between gap-4">
                ...
                <div className="flex items-center gap-3"><div className="hidden items-center gap-2 text-xs text-slate-400 sm:flex"><CalendarDays className="size-4 text-teal-300" /><span>Turno {empresa.turno}</span></div><button disabled={cargando || empresa.estado !== 'activa'} onClick={() => avanzarTurno()} className="inline-flex items-center gap-2 rounded-md border border-teal-300/40 bg-teal-300/10 px-3 py-2 text-xs font-medium text-teal-200 transition-colors hover:bg-teal-300/20 disabled:cursor-not-allowed disabled:opacity-50">Avanzar turno <ArrowRight className="size-3.5" /></button><span className="hidden rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1.5 text-xs text-emerald-300 sm:inline-flex">{empresa.estado}</span></div>
            </div>
